...
- 文字ボックスは文字モードでキャンバスをダブルクリックして作成します。未選択時は文字だけ表示し、選択後は枠と右下ハンドルでサイズ変更できます。
- プレビューでは、文字ボックスは基本的に文字だけを表示します。
- 文字ボックスと背景図面は、保存、読み込み、プレビューに反映されます。
- 図形テンプレートから、長方形、半円前面、楕円形、陸上トラック風などの会場パーツをサイズ入力で作成できます。
...
また、`テンプレート > 背景図面をテンプレート登録` から、背景PDF/画像をローカルテンプレートとして登録できます。
...
提出用プレビューでは、作業シート全体だけでなく、会場テンプレート、配置物、背景図面の範囲に合わせて表示できます。
...
- `テンプレート > 図形テンプレートを作成` から、長方形、半円前面、楕円形、陸上トラック風などの図形をサイズ入力で作成
